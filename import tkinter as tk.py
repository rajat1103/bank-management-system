...
def create_account():
    ...
    if not (acno and pan and name and acc_type and balance):
        ...
        return
    if not pan.isalnum():
        messagebox.showerror("Error", "PAN must be alphanumeric!")
        return
...
def show_balance():
    ...
    conn = connect_db()
    cursor = conn.cursor()
    cursor.execute("SELECT name, pan, type, balance FROM accounts WHERE acno = %s", (int(acno),))
...
def delete_account():
    ...
    conn = connect_db()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM accounts WHERE acno = %s", (int(acno),))
    if cursor.rowcount:
        conn.commit()
...
def modify_account():
    ...
    cursor.execute("UPDATE accounts SET pan = %s, name = %s, type = %s, balance = %s WHERE acno = %s",
                   (pan, name, acc_type, int(balance), int(acno)))
    if cursor.rowcount:
        conn.commit()
...
try:
    wallet_img = Image.open(r"C:\Users\HP\BANK MANAGEMENT SYSTEM\bank_icon.png")
    wallet_img = wallet_img.resize((500, 550))
    wallet_img = wallet_img.convert("RGBA")
    wallet_img.putalpha(120)
...
style.configure("TButton", font=("Times New Roman", 12), padding=5, background="#067662", foreground="black")
